...
from cryptall_2.core.main_modulo import V5_WITH_TABLE
...
def main():
    ...
    v5 = V5(text, char_ecncode_mod, d_mod_range)
    v5_with_table = V5_WITH_TABLE(text, char_ecncode_mod, d_mod_range)
    print(f"Generated array of size: {text.shape} bytes\n")

    start_time = time.perf_counter()
    encoded = v5_with_table.encode()
    ...
    start_time = time.perf_counter()
    decoded_vector = v5_with_table.decode(encoded, char_ecncode_mod, d_mod_range)
    end_time = time.perf_counter()
    execution_time = end_time - start_time
